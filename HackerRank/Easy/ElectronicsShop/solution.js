...
function getMoneySpent(keyboards, drives, b) {
  ...
  // Imediately check the lowest keyboard price and mouse price
  // If she can't afford it return -1
  if (Math.min(...keyboards) + Math.min(...drives) > b) return -1;

  // Brute force check to process the highest possible combination
  let highest = 0;
  keyboards.forEach(k => {
    drives.forEach(d => {
      if (k + d > highest && k + d <= b) {
        highest = k + d;
      }
    });
  });
